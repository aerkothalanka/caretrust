...
function FilterBar({ filters, values, setters, services }) {
  ...
  return <section className="filterCard">
    ...
    <label><span>Procedure</span><select multiple size="4" value={values.services} onChange={(e) => { const selected = Array.from(e.target.selectedOptions).map((o) => o.value); setters.setServices(selected.length ? selected : [services[0]?.service_id].filter(Boolean)); }}>{services.map((s) => <option key={s.service_id} value={s.service_id}>{s.service_label}</option>)}</select></label>
...
function FacilityTable({ facilities, selected, setSelected, onOpenTrust, onOpenReview }) {
  ...
  const [confidenceOp, setConfidenceOp] = useState('gte');
...
function App() {
  ...
  return <>
    ...
    <main className="main">
      ...
      <section className="hero filtersOnly"><FilterBar filters={filters} values={{ country, state, city, pincode, services: activeServices, radius }} setters={{ setCountry, setState, setCity, setPincode, setServices: setSelectedServices, setRadius }} services={services} /></section>
